...
def generate_message(action: str, category: str, merchant_id: str, merchant: Dict[str, Any], trigger: Dict[str, Any]) -> Dict[str, str]:
    ...
    if action == "fix_rating":
        variants = [
            "Responding to recent reviews today can quickly rebuild trust.",
            "Addressing customer feedback now can improve how new users perceive your business.",
            "Replying to recent reviews can help recover customer confidence quickly."
        ]
        
        # Deterministic selection based on merchant_id
        template_idx = sum(ord(c) for c in merchant_id) % len(variants)
        selected_template = variants[template_idx]
        
        if trigger_type in ["festival", "spike"]:
            # Acknowledge trigger context first, then flag the rating risk
            ...
            message = f"Your rating is at {rating} — a low score causes customers to skip your listing. {selected_template} Want me to draft those replies now?"
